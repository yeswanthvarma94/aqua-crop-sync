...
create policy if not exists "Members can view usernames of their account"
...
create policy if not exists "Owners can manage usernames"
...
create trigger if not exists update_usernames_updated_at
...
-- Helper: owner or manager check via membership role
-- (No function needed; we'll use inline EXISTS with role IN ('owner','manager'))
